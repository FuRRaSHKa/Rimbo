...
public class EnemyMoving : MonoBehaviour {
    ...
    private void PathfindingCheck() {
        ...
        directionToMove = new Vector2(Mathf.Cos(transform.eulerAngles.z * Mathf.PI / 180), Mathf.Sin(transform.eulerAngles.z * Mathf.PI / 180)).normalized * speed;

        direction = ((Vector2)path.vectorPath[currentPathPoint] - rgbd2D.position).normalized;

        if (Vector2.Distance(rgbd2D.position, path.vectorPath[currentPathPoint]) < minDistance) {
    ...
    private void FixedUpdate() {
        ...
        rgbd2D.velocity = directionToMove;
    ...
    private void PhysicsRotating() {
        ...
        if (rgbd2D.velocity != Vector2.zero) {
            ...
            float angle = Vector2.SignedAngle(directionToMove, direction);
